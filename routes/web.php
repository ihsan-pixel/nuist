...
Route::domain('spmb.nuist.id')->group(function () {
    ...
    Route::middleware(['guest'])->group(function () {
        ...
        Route::post('/login', function (\Illuminate\Http\Request $request) {
            ...
            return redirect($spmbSchoolHome);
        })->middleware('throttle:6,1');
        Route::get('/password/reset', [App\Http\Controllers\Auth\ForgotPasswordController::class, 'showLinkRequestForm']);
        Route::post('/password/email', [App\Http\Controllers\Auth\ForgotPasswordController::class, 'sendResetLinkEmail']);
        Route::get('/password/reset/{token}', [App\Http\Controllers\Auth\ResetPasswordController::class, 'showResetForm']);
    ...
    Route::middleware(['auth'])->group(function () use ($runSpmbController) {
        ...
        Route::prefix('ppdb/lp')->group(function () use ($runSpmbController) {
            ...
            Route::post('/jalur/{id}', function (\Illuminate\Http\Request $request, $id) use ($runSpmbController) {
                return $runSpmbController(AdminLPController::class, 'storeJalur', ['request' => $request, 'id' => $id]);
            });
            Route::put('/jalur/{jalurId}', function (\Illuminate\Http\Request $request, $jalurId) use ($runSpmbController) {
                return $runSpmbController(AdminLPController::class, 'updateJalur', ['request' => $request, 'jalurId' => $jalurId]);
            });
            Route::delete('/jalur/{jalurId}', function ($jalurId) use ($runSpmbController) {
                return $runSpmbController(AdminLPController::class, 'deleteJalur', ['jalurId' => $jalurId]);
            });
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::get('/presensi-admin/face-diagnostics', [App\Http\Controllers\Admin\FaceDiagnosticController::class, 'index'])->name('presensi_admin.face_diagnostics');
    ...
    Route::middleware(['role:super_admin,admin,pengurus,tenaga_pendidik'])->group(function () {
        ...
        Route::delete('teaching-schedules/school/{schoolId}/schedules', [App\Http\Controllers\TeachingScheduleController::class, 'destroySchoolSchedules'])->name('teaching-schedules.destroy-school-schedules');
...
Route::middleware(['auth'])->prefix('mobile')->name('mobile.')->group(function () {
    ...
    Route::get('/laporan/persentase-kehadiran/download', [App\Http\Controllers\Mobile\Laporan\LaporanController::class, 'downloadPersentaseKehadiranPdf'])->name('laporan.persentase-kehadiran.download');
...
Route::prefix('spp-siswa')->middleware(['auth', 'role:super_admin,admin_spp,pengurus'])->name('spp-siswa.')->group(function () {
    ...
    Route::post('/bni-va/export', [App\Http\Controllers\SppSiswaController::class, 'exportBniVa'])->name('bni-va.export');
...
Route::middleware(['auth', 'role:super_admin,admin,admin_spp,pengurus'])->prefix('data-sekolah')->name('data-sekolah.')->group(function () {
    ...
    Route::get('/data-siswa/export-complete', [App\Http\Controllers\DataSiswaController::class, 'exportComplete'])->name('data-siswa.export-complete');
...
Route::prefix('instumen-talenta')->name('instumen-talenta.')->middleware(['auth'])->group(function () {
    ...
    // Instrumen Penilaian pages (separate views)
    Route::get('/instrumen-penilaian', [App\Http\Controllers\InstumenTalentaController::class, 'instrumenPenilaian'])->name('instrumen-penilaian');
...
Route::middleware(['auth', 'role:super_admin,admin,pengurus,mgmp'])->prefix('mgmp')->name('mgmp.')->group(function () {
    ...
    Route::post('/academica/article-upload', [App\Http\Controllers\MGMPController::class, 'uploadAcademicaArticle'])->name('academica.article-upload');
    Route::post('/academica/reset-update', [App\Http\Controllers\MGMPController::class, 'storeAcademicaResetUpdate'])
        ->middleware(['throttle:10,1'])
