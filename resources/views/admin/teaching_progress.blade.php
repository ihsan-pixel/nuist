...
@section('title', 'Progress Mengajar')
...
<div class="container-fluid mgmp-page teaching-progress-page">
    <div class="mgmp-hero-strip mb-4">
        <div class="d-flex flex-column flex-xl-row justify-content-between gap-3 align-items-xl-end">
            ...
            <form method="GET" action="{{ route('admin.teaching_progress') }}" class="row g-2 align-items-end hero-filter-form">
                <input type="hidden" name="tab" value="{{ $activeTab }}">
                <input type="hidden" name="week" value="{{ $startOfWeek->format('o-\\WW') }}">
                <input type="hidden" name="teaching_recap_period" value="{{ $teachingRecapData['period'] }}">
                <input type="hidden" name="teaching_recap_week" value="{{ $teachingRecapData['week_value'] }}">
                <input type="hidden" name="teaching_recap_month" value="{{ $teachingRecapData['month_value'] }}">
    ...
    <div class="teaching-tab-switch mb-4">
        <a href="{{ $tabProgressUrl }}" class="tab-pill {{ $activeTab === 'progress' ? 'active' : '' }}">
            <i class="bx bx-bar-chart-alt-2"></i>
            <span>Progress Mengajar</span>
        </a>
        <a href="{{ $tabLaporanUrl }}" class="tab-pill {{ $activeTab === 'laporan' ? 'active' : '' }}">
                ...
                <div id="weeklyReportCollapse" class="accordion-collapse collapse show" aria-labelledby="weeklyReportHeading" data-bs-parent="#teachingReportsAccordion">
                    <div class="accordion-body">
                        <div class="section-toolbar mb-3">
                            <form method="GET" action="{{ route('admin.teaching_progress') }}" class="row g-2 align-items-end">
                                ...
                                <div class="col-md-3 col-lg-2">
                                    ...
                                    <input type="week" name="week" value="{{ $startOfWeek->format('o-\\WW') }}" class="form-control">
                                </div>
                                    ...
                                    <tr>
                                        ...
                                        @for($i = 0; $i < 6; $i++)
                                            <th colspan="5" class="text-center">
                                                {{ \Carbon\Carbon::parse($weeklyDayMarkers[$i]['date'])->locale('id')->translatedFormat('D, d M') }}
                                            </th>
                                        @endfor
                                ...
                                <tbody>
                                    @foreach($laporanData as $kabupaten)
                                        ...
                                        @foreach(collect($kabupaten['madrasahs'])->sortBy(fn ($madrasah) => (int) $madrasah['scod']) as $madrasah)
                                            <tr>
                                                ...
                                                @foreach($madrasah['presensi'] as $presensi)
                                                    <td class="text-center">{{ $presensi['jadwal'] }}</td>
                                                    <td class="text-center">{{ $presensi['hadir'] }}</td>
                                                    <td class="text-center">{{ $presensi['izin'] }}</td>
                                                    <td class="text-center">{{ $presensi['tidak_presensi_jurnal'] }}</td>
                                                    <td class="text-center">{{ $presensi['alpha'] }}</td>
                                                @endforeach
                                                <td class="text-center">{{ $madrasah['total_jadwal_berjalan'] }}</td>
                                        ...
                                        <tr class="report-total-row">
                                            <td colspan="3">TOTAL {{ $kabupaten['kabupaten'] }}</td>
                                    ...
                                    <tr class="report-grand-total">
                                        ...
                                        <td class="text-center">{{ $weeklyGrandSudah }}</td>
                                        <td class="text-center">{{ $weeklyGrandBelum }}</td>
                                        <td class="text-center">{{ $weeklyGrandTotal }}</td>
                                        @for($i = 0; $i < 6; $i++)
                                            @php $grandDay = collect($laporanData)->sum(fn ($kabupaten) => $kabupaten['daily_totals'][$i]['jadwal']); @endphp
                                            @if(!empty($weeklyDayMarkers[$i]['is_holiday']))
                                                <td class="text-center">-</td>
                                                <td class="text-center">-</td>
                                                <td class="text-center">-</td>
                                                ...
                                                <td class="text-center">-</td>
                                            @else
                                                <td class="text-center">{{ collect($laporanData)->sum(fn ($kabupaten) => $kabupaten['daily_totals'][$i]['jadwal']) }}</td>
                                                <td class="text-center">{{ collect($laporanData)->sum(fn ($kabupaten) => $kabupaten['daily_totals'][$i]['hadir']) }}</td>
                                                <td class="text-center">{{ collect($laporanData)->sum(fn ($kabupaten) => $kabupaten['daily_totals'][$i]['izin']) }}</td>
                                                <td class="text-center">{{ collect($laporanData)->sum(fn ($kabupaten) => $kabupaten['daily_totals'][$i]['tidak_presensi_jurnal']) }}</td>
                                                <td class="text-center">{{ collect($laporanData)->sum(fn ($kabupaten) => $kabupaten['daily_totals'][$i]['alpha']) }}</td>
                                            @endif
                                        @endfor
                                        <td class="text-center">{{ $weeklyGrandJadwal }}</td>
                                        <td class="text-center">{{ $weeklyGrandHadir }}</td>
                                        <td class="text-center">{{ $weeklyGrandIzin }}</td>
                            ...
                            <table class="table align-middle report-table">
                                ...
                                <tbody>
                                    ...
                                    <tr class="report-grand-total">
                                        ...
                                        <td class="text-center">{{ $monthlyGrandJadwal }}</td>
                        ...
                        <div class="section-toolbar mb-3">
                            <form method="GET" action="{{ route('admin.teaching_progress') }}" class="row g-2 align-items-end">
                                ...
                                <div class="col-md-3 col-lg-2" id="teachingRecapWeekWrapper">
                                    <label class="form-label">Pilih Minggu</label>
                                    <input type="week" name="teaching_recap_week" id="teachingRecapWeek" class="form-control" value="{{ $teachingRecapData['week_value'] }}">
                                </div>
                                <div class="col-md-3 col-lg-2" id="teachingRecapMonthWrapper">
                                    <label class="form-label">Pilih Bulan</label>
                                    <input type="month" name="teaching_recap_month" id="teachingRecapMonth" class="form-control" value="{{ $teachingRecapData['month_value'] }}">
                                </div>
                                <div class="col-md-auto">
                                    <button type="submit" class="btn btn-primary">
                                        <i class="bx bx-refresh me-1"></i> Perbarui
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-md-3">
                                <div class="mini-stat-card">
                                    ...
                                    <strong>{{ number_format($teachingRecapData['summary']['total_tenaga_pendidik']) }}</strong>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="mini-stat-card danger">
                                    <span>Belum Presensi Mengajar</span>
                                    <strong>{{ number_format($teachingRecapData['summary']['total_tidak_presensi']) }}</strong>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="mini-stat-card success">
                                    <span>Sudah Punya Jadwal</span>
                                    <strong>{{ number_format($teachingRecapData['summary']['total_sudah_jadwal']) }}</strong>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="mini-stat-card warning">
                                    <span>Belum Punya Jadwal</span>
                                    <strong>{{ number_format($teachingRecapData['summary']['total_belum_jadwal']) }}</strong>
                                </div>
                            </div>
                        </div>
                                ...
                                <tbody>
                                    @foreach($teachingRecapData['rows'] as $teacher)
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">{{ $teacher['total_izin'] }}</td>
                                            <td class="text-center">{{ $teacher['total_belum_presensi'] }}</td>
                                            <td class="text-center">{{ number_format($teacher['persentase_tidak_presensi'], 1) }}%</td>
                                            <td>
                                                <span class="table-badge {{ $statusClass }}">{{ $teacher['status_presensi'] }}</span>
                                            </td>
                                            <td>{{ $teacher['rincian_tanggal'] }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
    ...
    @else
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-start">
                            <div>
                                ...
                                <h3 class="mb-0">{{ number_format($schoolProgressData['summary']['total_schools']) }}</h3>
                            </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-start">
                            <div>
                                ...
                                <h3 class="mb-0">{{ number_format($schoolProgressData['summary']['schools_with_reports']) }}</h3>
                            </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-start">
                            <div>
                                ...
                                <h3 class="mb-0">{{ number_format($schoolProgressData['summary']['total_reports']) }}</h3>
                            </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-start">
                            <div>
                                ...
                                <h3 class="mb-0">{{ number_format($schoolProgressData['summary']['total_present_students']) }}</h3>
                            </div>
        ...
        <div class="card mgmp-panel mb-4">
            <div class="card-body p-4">
                <div class="d-flex flex-column flex-lg-row justify-content-between gap-3 align-items-lg-center">
                    <div>
                        <h5 class="mb-1">Data Progress {{ $schoolProgressData['month_label'] }}</h5>
                    ...
                    <div class="progress-summary-chip">
                        {{ number_format($schoolProgressData['summary']['reported_teachers']) }} guru tercatat mengirim laporan pada periode ini
                    </div>
                </div>
            </div>
        </div>

        @forelse($schoolProgressData['kabupaten_groups'] as $group)
            <div class="card mgmp-panel mb-4">
                <div class="card-header border-0 p-4 pb-0 bg-transparent">
                    <div class="d-flex flex-column flex-lg-row justify-content-between gap-3 align-items-lg-center">
                        <div>
                            <h5 class="mb-1">{{ $group['kabupaten'] }}</h5>
                            <p class="text-muted mb-0">{{ $group['schools_with_reports'] }} dari {{ $group['total_schools'] }} sekolah sudah mengirim {{ number_format($group['total_reports']) }} laporan.</p>
                        </div>
                        <span class="mgmp-chip">{{ $schoolProgressData['month_label'] }}</span>
                    </div>
                </div>
                <div class="card-body p-4">
                        ...
                        <table class="table align-middle">
                            ...
                            <tbody>
                                @foreach($group['schools'] as $school)
                                    @php $detailId = 'school-progress-' . $school['id']; @endphp
                                    <tr>
                                        <td class="fw-semibold">{{ $school['scod'] }}</td>
                                        <td>
                                            <div class="fw-semibold">{{ $school['name'] }}</div>
                                            <div class="text-muted school-subline">
                                                {{ $school['reported_teachers'] }} guru pelapor dari {{ $school['total_teachers'] }} guru
                                            </div>
                                        </td>
                                        <td class="text-center">{{ number_format($school['total_teachers']) }}</td>
                                        <td class="text-center">{{ number_format($school['total_reports']) }}</td>
                                        <td class="text-center">{{ number_format($school['total_days_reported']) }}</td>
                                        <td class="text-center">
                                            <div class="progress-ring-text">{{ number_format($school['progress_percentage'], 1) }}%</div>
                                        </td>
                                        <td class="text-center">{{ $school['latest_report_label'] ?? '-' }}</td>
                                        <td class="text-center">
                                            <button class="btn btn-outline-primary btn-sm px-3" type="button" data-bs-toggle="collapse" data-bs-target="#{{ $detailId }}" aria-expanded="false" aria-controls="{{ $detailId }}">
                                                Lihat Detail
                                            </button>
                                        </td>
                                    </tr>
                                                ...
                                                <div class="school-detail-panel">
                                                    <div class="school-detail-stats">
                                                        <div class="detail-stat">
                                                            ...
                                                            <strong>{{ number_format($school['scheduled_sessions']) }}</strong>
                                                        </div>
                                                        <div class="detail-stat">
                                                            <span>Laporan Terkirim</span>
                                                            <strong>{{ number_format($school['total_reports']) }}</strong>
                                                        </div>
                                                        <div class="detail-stat">
                                                            <span>Siswa Hadir</span>
                                                            <strong>{{ number_format($school['total_present_students']) }}</strong>
                                                        </div>
                                                        <div class="detail-stat">
                                                            <span>Hari KBM</span>
                                                            <strong>{{ $school['hari_kbm'] ?: '-' }}</strong>
                                                        </div>
                                                    </div>

                                                    @if(count($school['daily_reports']) > 0)
                                                        <div class="accordion daily-report-accordion" id="accordion-{{ $school['id'] }}">
                                                            @foreach($school['daily_reports'] as $dateIndex => $dailyReport)
                                                                @php
                                                                    $dailyHeadingId = 'daily-heading-' . $school['id'] . '-' . $dateIndex;
                                                                    $dailyCollapseId = 'daily-collapse-' . $school['id'] . '-' . $dateIndex;
                                                                @endphp
                                                                <div class="accordion-item mb-3">
                                                                    <h2 class="accordion-header" id="{{ $dailyHeadingId }}">
                                                                        <button class="accordion-button {{ $dateIndex === 0 ? '' : 'collapsed' }}" type="button" data-bs-toggle="collapse" data-bs-target="#{{ $dailyCollapseId }}" aria-expanded="{{ $dateIndex === 0 ? 'true' : 'false' }}" aria-controls="{{ $dailyCollapseId }}">
                                                                            <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center w-100 pe-3 gap-2">
                                                                                <div>
                                                                                    <div class="fw-semibold">{{ $dailyReport['day_label'] }}, {{ $dailyReport['date_label'] }}</div>
                                                                                </div>
                                                                                <div class="daily-report-meta">
                                                                                    <span>{{ $dailyReport['total_reports'] }} laporan</span>
                                                                                    <span>{{ $dailyReport['total_teachers'] }} guru</span>
                                                                                    <span>{{ number_format($dailyReport['total_present_students']) }} siswa hadir</span>
                                                                                </div>
                                                                            </div>
                                                                        </button>
                                                                    </h2>
                                                                    <div id="{{ $dailyCollapseId }}" class="accordion-collapse collapse {{ $dateIndex === 0 ? 'show' : '' }}" aria-labelledby="{{ $dailyHeadingId }}" data-bs-parent="#accordion-{{ $school['id'] }}">
                                                                        <div class="accordion-body">
                                                                            <div class="table-responsive detail-table-wrapper">
                                                                                <table class="table align-middle detail-table">
                                                                                    ...
                                                                                    <tbody>
                                                                                        @foreach($dailyReport['rows'] as $row)
                                                                                            <tr>
                                                                                                <td>{{ $loop->iteration }}</td>
                                                                                                <td>
                                                                                                ...
                                                                                                <td>
                                                                                                    <span class="table-badge {{ $row['status_class'] }}">{{ $row['status_label'] }}</span>
                                                                                                </td>
                                                                                                <td>{{ $row['student_summary'] }}</td>
                                                                                                <td>{{ $row['materi'] }}</td>
                                                                                            </tr>
                                                                                        @endforeach
                                                                                    </tbody>
                                                    ...
                                                    @else
                                                        <div class="mgmp-empty-state">
                                                            ...
                                                            <div>Data untuk sekolah ini belum tercatat pada periode {{ $schoolProgressData['month_label'] }}.</div>
                                                        </div>
                                                    @endif
                                                </div>
                                            </div>
