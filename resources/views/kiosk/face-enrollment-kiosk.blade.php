...
@section('title', 'Mode Kiosk 2 - Daftar Wajah')
    ...
    <div class="row g-4">
        ...
        <p class="text-secondary mb-4">Fokus ke 6 template capture wajah. Tidak ada presensi di layar ini.</p>
                <div class="mb-3">
                    ...
                    <form method="GET" action="{{ route('kiosk.face-enrollment.index') }}">
            ...
            <div class="panel p-4 h-100 camera-panel">
                ...
                <select id="teacherSelect" class="form-select mb-3">
                    ...
                    @foreach($teachers as $teacher)
                        <option value="{{ $teacher->id }}">{{ $teacher->name }}{{ $teacher->madrasah?->nama ? ' - '.$teacher->madrasah->nama : '' }}</option>
                    @endforeach
                </select>
...
<script>
window.__teachers = @json($teachersPayload);
window.__phases = @json($phases);
</script>
@endsection
...
<script src="{{ asset('js/face-recognition-v2.js') }}"></script>
<script>
(function () {
    ...
    const selectedMadrasahId = @json($selectedMadrasahId);
    ...
    async function startSession() {
        ...
        await initCamera();
        const response = await fetch(@json(route('kiosk.face-enrollment.sessions.start')), {
            method: 'POST',
            headers: {
    ...
    async function capturePhase(phase, phaseIndex) {
        ...
        const response = await fetch(@json(url('/kiosk-face-enrollment/sessions')) + `/${session.id}/captures`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': csrfToken,
    ...
    async function finishSession() {
        ...
        const response = await fetch(@json(url('/kiosk-face-enrollment/sessions')) + `/${session.id}/complete`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': csrfToken,
    ...
    async function resetSession() {
        ...
        resetBtn.disabled = true;
        try {
            const response = await fetch(@json(url('/kiosk-face-enrollment/sessions')) + `/${session.id}`, {
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': csrfToken,
                    'Accept': 'application/json',
