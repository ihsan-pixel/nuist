                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @if($item['tagihan_exists'])
                                            <span class="badge badge-modern bg-info ms-1">
                                                <i class="bx bx-check me-1"></i> Tagihan Sudah Dibuat
                                            </span>
                                        @else
                                            <button type="button" class="btn btn-success btn-sm ms-1" onclick="openBuatTagihanModal({{ $item['madrasah']->id }}, '{{ $item['madrasah']->name }}', {{ $item['total_tahunan'] }})">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control" id="modal_tahun_anggaran" name="tahun_anggaran"
                                       value="{{ request('tahun', date('Y')) }}" readonly>
                            </div>
                        </div>
                    </div>
                    <div class="row">
...
document.getElementById('buatTagihanForm').addEventListener('submit', function(e) {
    ...
    fetch('{{ route("uppm.store-tagihan") }}', {
        method: 'POST',
        body: formData,
        headers: {
