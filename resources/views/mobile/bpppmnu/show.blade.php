...
.bpp-detail-shell.bpp-scanner-only .bpp-detail-back,.bpp-detail-shell.bpp-scanner-only .bpp-detail-overview,.bpp-detail-shell.bpp-scanner-only .bpp-recap,.bpp-detail-shell.bpp-scanner-only .bpp-attendee-list{display:none!important}.bpp-detail-shell.bpp-scanner-only .bpp-detail-scanner{margin-top:8px;min-height:calc(100vh - 150px);display:flex!important;flex-direction:column;justify-content:center}.bpp-detail-shell.bpp-scanner-only #qr-video{display:block!important;min-height:260px;object-fit:cover}
...
<style>.bpp-attendee-list{background:#fff;border:1px solid #e6eae8;border-radius:12px;padding:14px;margin-top:12px}.bpp-attendee-list h2{font-size:12px;font-weight:600;margin:0 0 10px}.bpp-attendee-row{display:flex;justify-content:space-between;gap:10px;padding:10px 0;border-top:1px solid #edf0ed}.bpp-attendee-row:first-of-type{border-top:0}.bpp-attendee-name{font-size:11px;font-weight:600;color:#344e40;overflow-wrap:anywhere}.bpp-attendee-role{display:block;font-size:9px;color:#68736e;margin-top:2px}.bpp-attendee-status{font-size:9px;white-space:nowrap;color:#68736e}.bpp-attendee-status.is-present{color:#286248}</style>
<a class="bpp-detail-back" href="{{ route($bppRoutePrefix.'presensi') }}"><i class="bx bx-arrow-back" aria-hidden="true"></i>Kembali ke Agenda</a>
...
@if($event->attachment)<a href="{{ route($bppRoutePrefix.'events.attachment', $event) }}" class="bpp-detail-attachment"><i class="bx bx-download" aria-hidden="true"></i>&nbsp;Unduh lampiran undangan</a>@endif
</div>
<div class="bpp-recap" id="bpp-recap" data-url="{{ route($bppRoutePrefix.'events.recap', $event) }}"><div class="bpp-recap-item"><span class="bpp-recap-value" data-recap="present">{{ $recap['present'] }}</span><span class="bpp-recap-label">Sudah hadir</span></div><div class="bpp-recap-item"><span class="bpp-recap-value" data-recap="remaining">{{ $recap['remaining'] }}</span><span class="bpp-recap-label" data-recap="remaining-label">{{ now()->gt($event->attendance_close_at) ? 'Tidak hadir' : 'Belum hadir' }}</span></div><div class="bpp-recap-item"><span class="bpp-recap-value" data-recap="percentage">{{ $recap['percentage'] }}%</span><span class="bpp-recap-label">Kehadiran</span></div></div>
<div class="bpp-attendee-list" id="bpp-attendees"><h2>Daftar Peserta</h2>@foreach($recap['rows'] as $row)<div class="bpp-attendee-row"><div><span class="bpp-attendee-name">{{ $row->name }}</span><span class="bpp-attendee-role">{{ $row->jabatan ?: '—' }}</span></div><span class="bpp-attendee-status {{ $row->status === 'Hadir' ? 'is-present' : '' }}">{{ $row->status }}</span></div>@endforeach</div>
<script>setInterval(()=>{const r=document.getElementById('bpp-recap'),list=document.getElementById('bpp-attendees');if(!r)return;fetch(r.dataset.url,{headers:{Accept:'application/json'},credentials:'same-origin'}).then(x=>x.json()).then(d=>{r.querySelector('[data-recap="present"]').textContent=d.present;r.querySelector('[data-recap="remaining"]').textContent=d.remaining;r.querySelector('[data-recap="remaining-label"]').textContent=d.remaining_label;r.querySelector('[data-recap="percentage"]').textContent=d.percentage+'%';if(list)list.innerHTML='<h2>Daftar Peserta</h2>'+d.rows.map(row=>'<div class="bpp-attendee-row"><div><span class="bpp-attendee-name">'+row.name+'</span><span class="bpp-attendee-role">'+(row.jabatan||'—')+'</span></div><span class="bpp-attendee-status '+(row.status==='Hadir'?'is-present':'')+'">'+row.status+'</span></div>').join('')}).catch(()=>{});},10000);</script>
@if($attendance)
<div class="alert alert-success">✓ Sudah hadir · {{ $attendance->attended_at->format('d-m-Y H:i:s') }} WIB</div>
@elseif($event->isOpen())
<section id="scanner" class="bpp-detail-card bpp-detail-scanner" data-event-id="{{ $event->id }}" data-scan-url="{{ route($bppRoutePrefix.'events.scan', $event) }}"><div>
