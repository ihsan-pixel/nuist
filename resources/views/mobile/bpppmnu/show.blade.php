...
<div class="bpp-attendee-list" id="bpp-attendees"><h2>Daftar Peserta</h2>@foreach($recap['rows'] as $row)<div class="bpp-attendee-row"><div><span class="bpp-attendee-name">{{ $row->name }}</span><span class="bpp-attendee-role">{{ $row->jabatan ?: '—' }}</span></div><span class="bpp-attendee-status {{ $row->attended_at ? 'is-present' : '' }}">{{ $row->attended_at ? 'Hadir' : 'Belum Hadir' }}</span></div>@endforeach</div>
