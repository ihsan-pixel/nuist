            ...
            <div class="produk-intro">
                ...
                <div class="produk-metrics animate fade-up delay-1">
                    <div class="produk-metric">
                        <strong>{{ collect($productGroups)->sum(fn ($group) => count($group['products'])) + count($subdomainProducts) }}</strong>
                        <span>Total produk dan layanan</span>
                    </div>
                    <div class="produk-metric">
                        <strong>{{ collect($productGroups)->sum(fn ($group) => collect($group['products'])->where('status', 'LIVE')->count()) + collect($subdomainProducts)->where('status', 'LIVE')->count() }}</strong>
                        <span>Produk live dan siap digunakan</span>
                    </div>
                    <div class="produk-metric">
                        <strong>{{ count($subdomainProducts) }}</strong>
                ...
                <div class="produk-anchor-wrap animate fade-up delay-2">
                    <ul class="produk-anchor-list">
                        <li><a href="{{ route('landing.produk') }}#subdomain-aktif">Subdomain Aktif</a></li>
                        @foreach($productGroups as $index => $group)
                            <li><a href="{{ route('landing.produk') }}#group-{{ $index + 1 }}">{{ $group['title'] }}</a></li>
                        @endforeach
                    </ul>
                </div>
            </div>
                        ...
                        <article class="subdomain-card">
                            <div class="subdomain-card-head">
                                <span class="subdomain-icon">
                                    <i class='bx {{ $subdomainProduct['icon'] }}'></i>
                                </span>
                                <span class="subdomain-status">{{ $subdomainProduct['status'] }}</span>
                            </div>

                            <div>
                                <h4>{{ $subdomainProduct['name'] }}</h4>
                                <span class="subdomain-domain">{{ $subdomainProduct['domain'] }}</span>
                            </div>

                            <span class="subdomain-audience">{{ $subdomainProduct['audience'] }}</span>
                            <p>{{ $subdomainProduct['description'] }}</p>

                            <a href="{{ $subdomainProduct['url'] }}" target="_blank" rel="noopener noreferrer" class="subdomain-link">
                                Buka {{ $subdomainProduct['domain'] }}
