...
<div class="report-shell">
                    ...
                    @else
                        <div class="col-12">
                            <div class="px-3 py-2 rounded-3" style="background: rgba(255,255,255,0.12); border: 1px solid rgba(255,255,255,0.08);">
                                ...
                                <div class="fw-semibold text-white">{{ optional($madrasahOptions->first())->name ?? 'Madrasah Aktif' }}</div>
                            </div>
                        </div>
                    @endif
                </form>
            ...
            <div class="report-card report-stat">
                <div class="d-flex justify-content-between align-items-start gap-3">
                    <div>
                        ...
                        <p class="report-stat-value">{{ number_format($reportSummary['total_siswa'] ?? 0) }}</p>
                    </div>
            ...
            <div class="report-card report-stat">
                <div class="d-flex justify-content-between align-items-start gap-3">
                    <div>
                        ...
                        <p class="report-stat-value">{{ number_format($reportSummary['total_kelas'] ?? 0) }}</p>
                    </div>
            ...
            <div class="report-card report-stat">
                <div class="d-flex justify-content-between align-items-start gap-3">
                    <div>
                        ...
                        <p class="report-stat-value">{{ number_format($reportSummary['total_tagihan'] ?? 0) }}</p>
                    </div>
            ...
            <div class="report-card report-stat">
                <div class="d-flex justify-content-between align-items-start gap-3">
                    <div>
                        ...
                        <p class="report-stat-value">{{ number_format($paidPercentage, 1) }}%</p>
                    </div>
                ...
                <div class="card-body p-4">
                    ...
                    @forelse($classSummary as $row)
                        <div class="report-class-item">
                            <div>
                                <div class="fw-semibold text-dark">{{ $row->kelas ?: '-' }}</div>
                                <div class="small text-muted">Kelas aktif pada laporan</div>
                            </div>
                            <span class="badge bg-light text-dark border">{{ number_format($row->jumlah_siswa) }} siswa</span>
                        </div>
                    @empty
                    ...
                    <div class="row g-3">
                        <div class="col-md-4">
                            <div class="rounded-4 p-3" style="background: #f5faf8;">
                                ...
                                <div class="h4 mb-0 text-dark">Rp {{ number_format($nominalTagihan, 0, ',', '.') }}</div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="rounded-4 p-3" style="background: #f7f9ff;">
                                <div class="report-stat-label mb-2">Sudah Terbayar</div>
                                <div class="h4 mb-0 text-dark">Rp {{ number_format($nominalTerbayar, 0, ',', '.') }}</div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="rounded-4 p-3" style="background: #fff8f2;">
                                <div class="report-stat-label mb-2">Outstanding</div>
                                <div class="h4 mb-0 text-dark">Rp {{ number_format($outstandingNominal, 0, ',', '.') }}</div>
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="report-progress"><span style="width: {{ min(100, $paidPercentage) }}%;"></span></div>
                            <div class="small text-muted mt-2">{{ number_format($paidPercentage, 1) }}% nominal tagihan sudah tercatat sebagai pembayaran berhasil.</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="report-card">
        <div class="card-body p-4">
                ...
                <table class="table table-bordered align-middle mb-0">
                    ...
                    @forelse($reportRows as $row)
                        ...
                        <tr>
                            ...
                            <td><span class="badge bg-success-subtle text-success">{{ number_format($row->tagihan_lunas) }}</span></td>
                            <td><span class="badge bg-warning-subtle text-warning">{{ number_format($row->tagihan_belum_lunas) }}</span></td>
                            <td>Rp {{ number_format($rowNominal, 0, ',', '.') }}</td>
                            <td>Rp {{ number_format($rowTerbayar, 0, ',', '.') }}</td>
                            <td style="min-width: 180px;">
                                <div class="report-progress mb-2"><span style="width: {{ min(100, $rowPercentage) }}%;"></span></div>
                                <div class="small text-muted">{{ number_format($rowPercentage, 1) }}% terbayar</div>
                            </td>
                        </tr>
                    @empty
                        <tr>
            ...
            <div class="mt-4">
                {{ $reportRows->links('pagination::bootstrap-5') }}
            </div>
        </div>
    </div>
</div>
