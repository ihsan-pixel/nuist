...
class _PengurusDashboardState extends State<_PengurusDashboard> {
  ...
  void _retry() => setState(() => _future = widget.repository.dashboard());
  @override Widget build(BuildContext context) => FutureBuilder<Map<String, dynamic>>(future: _future, builder: (context, snapshot) {
    ...
    if (snapshot.hasError) return _LoadError(onRetry: _retry, message: snapshot.error.toString());
    if (!snapshot.hasData) return _LoadError(onRetry: _retry, message: 'Data dashboard tidak tersedia.');
...
class _SchoolMonitorState extends State<_SchoolMonitor> {
  ...
  @override Widget build(BuildContext context) => FutureBuilder<Map<String, dynamic>>(
    ...
    builder: (context, snapshot) {
      if (snapshot.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator());
      if (snapshot.hasError) return _LoadError(onRetry: () => setState(() => _future = widget.repository.schools()), message: snapshot.error.toString());
      if (!snapshot.hasData) return _LoadError(onRetry: () => setState(() => _future = widget.repository.schools()), message: 'Data sekolah tidak tersedia.');
